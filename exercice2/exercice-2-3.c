...
void *client(void *arg) {
    ...
    while (barberSleeping) {
        // Attente active
        pthread_cond_wait(&clientReady, &mutex);  // Attend que le barbier soit prêt
    }
...
void *barber(void *arg) {
    ...
    while (1) {
        ...
        pthread_cond_signal(&clientReady);  // Signal au client que le barbier a fini
        pthread_mutex_unlock(&mutex);
    }
...
int main() {
    ...
    // Détruisez le mutex et la condition
